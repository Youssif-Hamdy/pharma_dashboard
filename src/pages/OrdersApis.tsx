...
export default function OrdersApis() {
  ...
  const statusById = useMemo(
    () =>
      orders.reduce<Record<string, OrderStatus>>(
        (acc, o) => ({ ...acc, [o._id]: o.status }),
        {},
      ),
    [orders],
  );
              ...
              {orders.map((o) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <span
                      className={`text-[11px] px-2.5 py-1 rounded-full border font-medium ${statusBadgeClass(o.status)}`}
                    >
                ...
                <tbody>
                  {orders.map((o) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600 max-w-[340px]">
                        {o.productName}
                      </td>
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">
                        {o.price === null ? "—" : `${o.price} ج.م`}
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full border font-medium ${statusBadgeClass(o.status)}`}
                        >
                          {statusLabel(o.status)}
                        </span>
                      </td>
          ...
          >
            ...
            <div className="rounded-xl border border-gray-200 px-3 py-2.5 bg-white">
              <div className="flex items-center justify-between mb-2">
                <span
                  className={`text-[11px] px-2 py-0.5 rounded-full border font-medium ${statusBadgeClass(draftStatus)}`}
                >
            ...
            {activeOrder.location && (
              <div className="mt-4">
                <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-2">
                  <MapPin size={13} />
                  {activeOrder.location.lat.toFixed(4)},{" "}
                  {activeOrder.location.lng.toFixed(4)}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setActiveOrder((prev) =>
                      prev ? { ...prev, status: draftStatus } : prev,
                    );
                  }}
                  disabled={
                    updatingId === activeOrder._id ||
